...
def rho_term_predict(infile = None, outfile = None, quiet = True):
    """
    ...
    ----------
    infile : str
        Path to the input Genome sequences file in fasta format
    outfile : str
        Path to the output file. A xlsx file containing Rho-dependent terminators "
        coordinates and a txt file containing informations about them
    """
    ...
    if not os.path.isfile(file_genome):
    ...
    for sq, genome in sequences_to_analyze.items():
        genome = genome.upper()
        ...
        with (open(f"{outfile}RhoTermPredict_{sq}.txt", "a", encoding="utf-8") as p,
              open(f"{outfile}RhoTermPredict_{sq}.csv", mode="a", encoding="utf-8") as predictions_file):
...
if __name__ == "__main__":
    ...
    in_file = input("Enter the name of the file containing the genome: ")
    out_file = input("Enter the name of the filepath to save the outputs: ")
    rho_term_predict(infile=in_file, outfile=out_file, quiet=False)
